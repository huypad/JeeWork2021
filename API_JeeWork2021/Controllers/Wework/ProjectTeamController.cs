...
namespace JeeWork_Core2021.Controllers.Wework
{
    ...
    public class ProjectTeamController : ControllerBase
    {
        ...
        public ProjectTeamController(IOptions<JeeWorkConfig> config, IHostingEnvironment hostingEnvironment)
        {
        ...
        {
            //string Token = lc.GetHeader(Request.Headers);
            //string Token = "f3d23d99-8342-49fe-afbb-211f525cae73";
            ...
            try
            {
                ...
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    //        "where p1.Disabled = 0 and id_user = " + loginData.UserID + ")) and p.Disabled = 0 and de.Disabled = 0 and ");
                    //}
                    //else
                    ...
                    //{
                    //    var info = DataAccount.Where(x => item["CreatedBy"].ToString().Contains(x.UserId.ToString()));
                    //    if (info.Count() > 0)
                    //    {
                    //        foreach (var u in info)
                    //        {
                    //            item["NguoiTao"] = u.Username;
                    //        }
                    //    }
                    //}
                    foreach (DataRow item in ds.Tables[1].Rows)
                ...
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    var data = (from r in ds.Tables[0].AsEnumerable()
                                select new
                                {
                                    id_row = r["id_row"],
                                    title = r["title"],
                                    icon = r["icon"] == DBNull.Value ? "" : (_config.LinkAPI + JeeWorkConstant.RootUpload + r["icon"]),
                                    description = r["description"],
                                    detail = r["detail"],
                                    id_department = r["id_department"],
                                    department = r["department"],
            ...
            try
            {
                ...
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    if (data.icon != null)
                    {
                        string x = "";
                        string folder = "/logo/";
                        if (!UploadHelper.UploadFile(data.icon.strBase64, data.icon.filename, folder, _hostingEnvironment.ContentRootPath, ref x))
                    ...
                    if (!WeworkLiteController.log(cnn, 31, idc, iduser, data.title))
                    {
                        cnn.RollbackTransaction();
                        return JsonResultCommon.Exception(cnn.LastError, _config, loginData.CustomerID, ControllerContext);
                    }
                    // Tạo 4 status mặc định cho project này
                    //string[,] list_status = {
                    //    {"Công việc mới tạo", "1", "rgb(211, 211, 211)", "0","0","0"},
                    //    {"Đang làm", "2","rgb(219, 177, 37)","0","0","1"},
                    //    {"Trễ hạn", "3","rgb(228, 59, 59)","0","1","0"},
                    //    {"Hoàn thành", "6","rgb(62, 158, 35)","1","0","0"},
                    //    };
                    //val = new Hashtable();
                    //for (int i = 0; i < list_status.GetLength(0); i++)
                    //{
                    //    val = new Hashtable();
                    //    val.Add("StatusName", list_status[i, 0]);
                    //    val.Add("Position", list_status[i, 1]);
                    //    val.Add("color", list_status[i, 2]);
                    //    val.Add("IsFinal", list_status[i, 3]);
                    //    val.Add("IsDeadline", list_status[i, 4]);
                    //    val.Add("IsToDo", list_status[i, 5]);
                    //    val.Add("Disabled", 0);
                    //    val.Add("id_project_team", idc);
                    //    val.Add("Type", 1);
                    //    val.Add("IsDefault", 1);
                    //    val.Add("CreatedDate", DateTime.Now);
                    //    val.Add("CreatedBy", iduser);
                    //    if (cnn.Insert(val, "we_status") <= 0)
                    //    {
                    //        cnn.RollbackTransaction();
                    //        cnn.Disconnect();
                    //        return false;
                    ...
                    WeworkLiteController.mailthongbao(idc, data.Users.Where(x => x.admin).Select(x => x.id_user).ToList(), 6, loginData, _config);//thiết lập vai trò admin
                    ...
                    #endregion
                    WeworkLiteController.mailthongbao(idc, data.Users.Where(x => !x.admin).Select(x => x.id_user).ToList(), 5, loginData, _config);//thêm vào dự án
        ...
        public async Task<object> Insert_Quick([FromBody] ProjectTeamModel data)
        {
            ...
            try
            {
                ...
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    WeworkLiteController.mailthongbao(idc, users_admin, 6, loginData, _config);//thiết lập vai trò admin
                    #region Notify thiết lập vai trò admin
                    for (int i = 0; i < users_admin.Count; i++)
                    {
                        NotifyModel notify_model = new NotifyModel();
                    ...
                    foreach (DataRow item in dt_member.Rows)
                    {
                        ...
                        WeworkLiteController.mailthongbao(idc, users_member, 5, loginData, _config);//thêm vào dự án
                        #region Notify thiết lập vai trò member
                        for (int i = 0; i < users_member.Count; i++)
                        {
                            NotifyModel notify_model = new NotifyModel();
        ...
        public async Task<object> Update([FromBody] ProjectTeamModel data)
        {
            ...
            try
            {
                ...
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    if (!WeworkLiteController.log(cnn, 32, data.id_row, iduser))
                    {
                        cnn.RollbackTransaction();
                        return JsonResultCommon.Exception(cnn.LastError, _config, loginData.CustomerID, ControllerContext);
                    }
                    cnn.EndTransaction();
                    WeworkLiteController.mailthongbao(data.id_row, data.Users.Where(x => x.id_row == 0 && x.admin).Select(x => x.id_user).ToList(), 6, loginData, _config);//thiết lập vai trò admin
                    Hashtable has_replace = new Hashtable();
                    List<long> users_admin = data.Users.Where(x => x.id_row == 0 && x.admin).Select(x => x.id_user).ToList();
                    List<long> users_member = data.Users.Where(x => x.id_row == 0 && !x.admin).Select(x => x.id_user).ToList();
                    #region Notify thiết lập vai trò admin
                    ...
                    #endregion
                    WeworkLiteController.mailthongbao(data.id_row, data.Users.Where(x => x.id_row == 0 && !x.admin).Select(x => x.id_user).ToList(), 5, loginData, _config);//thêm vào dự án
                    #region Notify thiết lập vai trò member
                    for (int i = 0; i < users_member.Count; i++)
                    {
                        NotifyModel notify_model = new NotifyModel();
                    ...
                    if (email_update_team)
                    {
                        ...
                        if (id_template > 0)
                        {
                            WeworkLiteController.mailthongbao(data.id_row, data.Users.Where(x => x.id_row > 0).Select(x => x.id_user).ToList(), id_template, loginData, _config, old);
                            #region Notify thiết lập vai trò member
                            for (int i = 0; i < users_member.Count; i++)
                            {
                                NotifyModel notify_model = new NotifyModel();
        ...
        public async Task<object> Update_Stage(ProjectTeamModel data)
        {
            ...
            try
            {
                ...
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    if (!WeworkLiteController.log(cnn, 34, data.id_row, iduser, "", old.Rows[0]["status"], data.status))
                    {
                        cnn.RollbackTransaction();
                        return JsonResultCommon.Exception(cnn.LastError, _config, loginData.CustomerID, ControllerContext);
                    }
        ...
        public async Task<object> UpdateByKey(long id, string key, string value)
        {
            ...
            try
            {
                ...
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    if (!WeworkLiteController.log(cnn, 32, id, iduser, ""))
                    {
                        cnn.RollbackTransaction();
                        return JsonResultCommon.Exception(cnn.LastError, _config, loginData.CustomerID, ControllerContext);
                    }
        ...
        public async Task<object> Close([FromBody] ProjectTeamCloseModel data)
        {
            ...
            try
            {
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    if (!WeworkLiteController.log(cnn, 34, data.id_row, iduser, "", old.Rows[0]["status"], data.close_status))
                    {
                        cnn.RollbackTransaction();
                        return JsonResultCommon.Exception(cnn.LastError, _config, loginData.CustomerID, ControllerContext);
                    }
        ...
        public BaseModel<object> Delete(long id)
        {
            ...
            try
            {
                ...
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    if (!WeworkLiteController.log(cnn, 33, id, iduser))
                    {
                        cnn.RollbackTransaction();
                        return JsonResultCommon.Exception(cnn.LastError, _config, loginData.CustomerID, ControllerContext);
                    }
        ...
        public BaseModel<object> ChangeType(long id)
        {
            ...
            try
            {
                ...
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    if (!WeworkLiteController.log(cnn, 35, id, iduser, "", !value, value))
                    {
                        cnn.RollbackTransaction();
                        return JsonResultCommon.Exception(cnn.LastError, _config, loginData.CustomerID, ControllerContext);
                    }
        ...
        public async Task<object> Duplicate([FromBody] ProjectTeamDuplicateModel data)
        {
            ...
            try
            {
                ...
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    if (!WeworkLiteController.log(cnn, 31, idc, iduser))
                    {
                        cnn.RollbackTransaction();
                        return JsonResultCommon.Exception(cnn.LastError, _config, loginData.CustomerID, ControllerContext);
                    }
...
join we_project_team p on p.id_row=u.id_project_team and p.id_row=" + id + " where u.disabled=0 and u.Id_user in (" + listID + " )";
            ...
            try
            {
                ...
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    foreach (var owner in datas)
                    {
                        ...
                        if (!WeworkLiteController.log(cnn, 36, data.id_row, iduser, null, owner.id_user))
                        {
                            cnn.RollbackTransaction();
                            return JsonResultCommon.Exception(cnn.LastError, _config, loginData.CustomerID, ControllerContext);
                        }
                    ...
                    {
                        WeworkLiteController.mailthongbao(data.id_row, users_admin, 6, loginData, _config);//thiết lập vai trò admin
                        for (int i = 0; i < users_admin.Count; i++)
                        {
                            NotifyModel notify_model = new NotifyModel();
                            has_replace = new Hashtable();
                    ...
                    {
                        WeworkLiteController.mailthongbao(data.id_row, users_member, 5, loginData, _config);//thêm vào dự án
                        for (int i = 0; i < users_member.Count; i++)
                        {
                            NotifyModel notify_model = new NotifyModel();
                            has_replace = new Hashtable();
        ...
        public BaseModel<object> UpdateUser(long id, bool admin)
        {
            ...
            try
            {
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    if (!WeworkLiteController.log(cnn, 37, id, loginData.UserID, "", !admin, admin))
                    {
                        cnn.RollbackTransaction();
                        return JsonResultCommon.Exception(cnn.LastError, _config, loginData.CustomerID, ControllerContext);
                    }
                    ...
                    WeworkLiteController.mailthongbao(id_project_team, users, id_template, loginData, _config);
                    #region Notify thiết lập vai trò member
        ...
        public async Task<object> Add_View(ProjectViewsModel data)
        {
            ...
            try
            {
                ...
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    if (!WeworkLiteController.log(cnn, 48, data.id_row, iduser, null, iduser))
                    {
                        cnn.RollbackTransaction();
                        return JsonResultCommon.Exception(cnn.LastError, _config, loginData.CustomerID, ControllerContext);
                    }
        ...
        public async Task<object> update_view(ProjectViewsModel data)
        {
            ...
            try
            {
                ...
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    if (!WeworkLiteController.log(cnn, 49, data.id_row, iduser, null, iduser))
                    {
                        cnn.RollbackTransaction();
                        return JsonResultCommon.Exception(cnn.LastError, _config, loginData.CustomerID, ControllerContext);
                    }
        ...
        public BaseModel<object> Delete_View(long id)
        {
            ...
            try
            {
                ...
                using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
                {
                    ...
                    if (!WeworkLiteController.log(cnn, 50, id, iduser))
                    {
                        cnn.RollbackTransaction();
                        return JsonResultCommon.Exception(cnn.LastError, _config, loginData.CustomerID, ControllerContext);
                    }
        ...
        private void mailthongbao(long id, int id_template, UserJWT loginData, DataTable dtOld = null)
        {
            using (DpsConnection cnn = new DpsConnection(_config.ConnectionString))
            {
                string str = $"select id_nv,holot+' '+ten as hoten, email from {_config.HRCatalog}.dbo.Tbl_Nhanvien nv " +
                    "join we_project_team_user u on u.disabled=0 " +
                    "and u.id_user=nv.id_nv " +
                    "where u.id_project_team=" + id;
                DataTable dtUser = cnn.CreateDataTable(str);
                cnn.Disconnect();
                WeworkLiteController.NotifyMail(id_template, id, loginData, dtUser, _config, dtOld);
            }
        }
        #endregion
