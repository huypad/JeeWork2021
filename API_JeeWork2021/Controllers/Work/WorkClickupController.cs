        ...
        public async Task<object> WorkFilter([FromQuery] QueryParams query)
        {
            ...
            try
            {
                ...
                using (DpsConnection cnn = new DpsConnection(ConnectionString))
                {
                    ...
                    ds_workspace = Common.GetWorkSpace(loginData, id, type);
                    ...
                    DataTable dt_fields = WeworkLiteController.ListField(id, type, cnn);
                    var data = from r in workspace.AsEnumerable()
                               select new
                               {
                                   ...
                                   color = r["color"],
                                   id_row = r["id_row"],
                                   project_data = GetWorkByProjects(r["id_row"].ToString(), dt_fields, ConnectionString, query, loginData, DataAccount, type, id)
                               };
                    return JsonResultCommon.ThanhCong(data, pageModel, Visible);
                }
            }
                ...
                using (DpsConnection cnn = new DpsConnection(ConnectionString))
                {
                    ...
                    for (int i = 0; i < data_import.dtW.Rows.Count; i++)
                    {
                        ...
                        if (dr["disabled"] == DBNull.Value || !(bool)dr["disabled"])
                        {
                            ...
                            DataTable dt_status = WeworkLiteController.StatusDynamic(data.id_project_team, new List<AccUsernameModel>(), cnn);
                        ...
                        if ("clickup_prioritize".Equals(data.key))
                        {
                            ...
                            if (WeworkLiteController.CheckNotify_ByConditions(id_project_team, "email_update_work", false, ConnectionString))
                            {
                                ...
                                if (dt_user.Rows.Count > 0)
                                {
                                    ...
                                    cnn.EndTransaction();
                                    #region Lấy thông tin để thông báo
                                    SendNotifyModel noti = WeworkLiteController.GetInfoNotify(25, ConnectionString);
                                    #endregion
                        ...
                        if ("estimates".Equals(data.key))
                        {
                            ...
                            if (WeworkLiteController.CheckNotify_ByConditions(id_project_team, "email_update_work", false, ConnectionString))
                            {
                                ...
                                if (dt_user.Rows.Count > 0)
                                {
                                    ...
                                    #region Lấy thông tin để thông báo
                                    SendNotifyModel noti = WeworkLiteController.GetInfoNotify(26, ConnectionString);
                                    #endregion
                                    #region Notify chỉnh sửa công việc
                                    Hashtable has_replace = new Hashtable();
                                    for (int i = 0; i < users.Count; i++)
                        ...
                        if (data.key == "deadline")
                        {
                            ...
                            if (WeworkLiteController.CheckNotify_ByConditions(id_project_team, "email_update_work", false, ConnectionString))
                            {
                                ...
                                if (dt_user.Rows.Count > 0)
                                {
                                    ...
                                    #region Lấy thông tin để thông báo
                                    SendNotifyModel noti = WeworkLiteController.GetInfoNotify(templateguimail, ConnectionString);
                                    #endregion
                                    #region Notify chỉnh sửa công việc
                                    Hashtable has_replace = new Hashtable();
                                    for (int i = 0; i < users.Count; i++)
                        ...
                        if (data.key == "start_date")
                        {
                            ...
                            if (WeworkLiteController.CheckNotify_ByConditions(id_project_team, "email_update_work", false, ConnectionString))
                            {
                                ...
                                if (dt_user.Rows.Count > 0)
                                {
                                    ...
                                    WeworkLiteController.mailthongbao(data.id_row, users, idtemplateguimail, loginData, ConnectionString, _notifier, _configuration, old);
                                    #region Lấy thông tin để thông báo
                                    SendNotifyModel noti = WeworkLiteController.GetInfoNotify(idtemplateguimail, ConnectionString);
                                    #endregion
                                    #region Notify chỉnh sửa công việc
                                    Hashtable has_replace = new Hashtable();
                                    for (int i = 0; i < users.Count; i++)
                        ...
                        if (data.key == "description")
                        {
                            ...
                            if (WeworkLiteController.CheckNotify_ByConditions(id_project_team, "email_update_work", false, ConnectionString))
                            {
                                ...
                                if (dt_user.Rows.Count > 0)
                                {
                                    ...
                                    #region Lấy thông tin để thông báo
                                    SendNotifyModel noti = WeworkLiteController.GetInfoNotify(28, ConnectionString);
                                    #endregion
                                    #region Notify chỉnh sửa công việc
                                    Hashtable has_replace = new Hashtable();
                                    for (int i = 0; i < users.Count; i++)
                        ...
                        if (data.key == "result")
                        {
                            ...
                            if (WeworkLiteController.CheckNotify_ByConditions(id_project_team, "email_update_work", false, ConnectionString))
                            {
                                ...
                                if (dt_user.Rows.Count > 0)
                                {
                                    ...
                                    #region Lấy thông tin để thông báo
                                    SendNotifyModel noti = WeworkLiteController.GetInfoNotify(31, ConnectionString);
                                    #endregion
                                    #region Notify chỉnh sửa công việc
                                    Hashtable has_replace = new Hashtable();
                                    for (int i = 0; i < users.Count; i++)
                            ...
                            if (dt_user.Rows.Count > 0)
                            {
                                ...
                                if (WeworkLiteController.CheckNotify_ByConditions(id_project_team, "email_update_status", false, ConnectionString))
                                {
                                    ...
                                    if (dts.Rows.Count > 0)
                                    {
                                        ...
                                        #region Lấy thông tin để thông báo
                                        SendNotifyModel noti = WeworkLiteController.GetInfoNotify(21, ConnectionString);
                                        #endregion
                            ...
                            if (data.value != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (dtCheck.Rows.Count > 0 && data.key == "assign")
                                    {
                                        ...
                                        if (cnn.ExecuteNonQuery("update we_work_user set Disabled = 1 where id_work = @id_work and loai = @loai", sqlcond123) < 0)
                                        {
                                            cnn.RollbackTransaction();
                                            return JsonResultCommon.Exception(_logger, cnn.LastError, _config, loginData, ControllerContext);
                                        }
                                        ...
                                        if (WeworkLiteController.CheckNotify_ByConditions(id_project_team, "email_update_work", false, ConnectionString))
                                        {
                                            ...
                                            #region Lấy thông tin để thông báo
                                            SendNotifyModel noti = WeworkLiteController.GetInfoNotify(22, ConnectionString);
                                            #endregion
                                        ...
                                        //else
                                        //{
                                        //    return JsonResultCommon.Custom("Công việc đã có người thực hiện, vui lòng tạo công việc con");
                                        //}
                                    }
                                    if (cnn.Insert(val1, "we_work_user") != 1)
                                    {
                                ...
                                if (WeworkLiteController.CheckNotify_ByConditions(id_project_team, "email_update_work", false, ConnectionString))
                                {
                                    ...
                                    WeworkLiteController.mailthongbao(data.id_row, users, idtemplatemail, loginData, ConnectionString, _notifier, _configuration);
                                    #region Lấy thông tin để thông báo
                        ...
                        if (data.key == "Tags")//thêm 1 tag mới
                        {
                            ...
                            if (WeworkLiteController.CheckNotify_ByConditions(id_project_team, "email_update_work", false, ConnectionString))
                            {
                                ...
                                if (dt_user.Rows.Count > 0)
                                {
                                    var users = new List<long> { long.Parse(dt_user.Rows[0]["id_nv"].ToString()) };
                                    cnn.EndTransaction();
                                    WeworkLiteController.mailthongbao(data.id_row, users, 34, loginData, ConnectionString, _notifier, _configuration, old);
                                    #region Lấy thông tin để thông báo
                                    SendNotifyModel noti = WeworkLiteController.GetInfoNotify(34, ConnectionString);
                                    #endregion
                                    #region Notify chỉnh sửa công việc
                                    Hashtable has_replace = new Hashtable();
                                    for (int i = 0; i < users.Count; i++)
                ...
                using (DpsConnection cnn = new DpsConnection(ConnectionString))
                {
                    ...
                    if (WeworkLiteController.CheckNotify_ByConditions(id_project, "email_delete_work", false, ConnectionString))
                    {
                        if (dt_user.Rows.Count > 0)
                        {
                            ...
                            #region Lấy thông tin để thông báo
                            SendNotifyModel noti = WeworkLiteController.GetInfoNotify(15, ConnectionString);
                            #endregion
        ...
        public async Task<object> Close(long id, bool closed = true)
        {
            ...
            try
            {
                ...
                using (DpsConnection cnn = new DpsConnection(ConnectionString))
                {
                    ...
                    if (old == null || old.Rows.Count == 0)
                        return JsonResultCommon.KhongTonTai("Công việc");
                    if ((bool)old.Rows[0]["closed"] == closed)
        ...
        public static DataSet GetWorkByEmployee(DpsConnection cnn, QueryParams query, long curUser, List<AccUsernameModel> DataAccount, string dieukien_where = "")
        {
            ...
            string sqlq = @$"select  distinct w.id_row,w.title,w.description,w.id_project_team,w.id_group
                          ...
                          ,isnull((select count(*) from we_work v where deadline < getdate() and deadline is not null and w.id_row = v.id_row and v.end_date is null),0) as TreHan-- Trễ hạn: Ngày kết thúc is null và deadline is not null và deadline < getdate()
                            ,isnull((select count(*) from we_work v where deadline < getdate() and deadline is not null and w.id_row = v.id_row and v.end_date is not null),0) as HT_QuaHan--Hoàn thành: Ngày kết thúc is not null và deadline is not null và deadline < getdate()
...
where u.disabled = 0 and u.loai = 2";
        ...
        private object GetWorkByProjects(string id_project_team, DataTable dt_fields, string ConnectionString, QueryParams query, UserJWT loginData, List<AccUsernameModel> DataAccount, long type, long id)
        {
            try
            {
                using (DpsConnection cnn = new DpsConnection(ConnectionString))
                    ...
                    if (!string.IsNullOrEmpty(query.filter["groupby"]))
                    {
                        ...
                        switch (groupby)
                        {
                            ...
                            case "assignee":
                                {
                                    dt_filter_tmp = new DataTable();
                                    dt_filter_tmp = cnn.CreateDataTable("select id_user as id_row from we_project_team_user where disabled = 0 and id_project_team = " + id_project_team);
                                    dt_filter_tmp.Columns.Add("title");
                                    dt_filter_tmp.Columns.Add("color");
                                    dt_filter_tmp.Columns.Add("Follower");
                                    dt_filter_tmp.Columns.Add("Description");
                    ...
                    if (result.Rows.Count > 0)
                    {
                        foreach (DataRow dr in result.Rows)
                        {
                            ...
                            if (string.IsNullOrEmpty(dr["id_parent"].ToString()))
                            {
                                string qs = sql_user + " and id_work in (select id_row from we_work where id_parent = " + dr["id_row"] + ")";
                                dr["UserSubtask"] = cnn.CreateDataTable(sql_user + " and id_work in (select id_row from we_work where id_parent = " + dr["id_row"] + ")", "(where)", cond);
                                dr["DataChildren"] = dtChildren(dr["id_row"].ToString(), result, ConnectionString, dt_fields, id_project_team.ToString(), DataAccount, user_child, loginData);
                                dr["DataStatus"] = list_status_user(dr["id_row"].ToString(), id_project_team.ToString(), loginData, ConnectionString, DataAccount);
                            }
                        }
