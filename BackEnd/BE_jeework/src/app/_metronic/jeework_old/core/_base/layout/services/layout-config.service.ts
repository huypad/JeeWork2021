...
export class LayoutConfigService {
	...
	async Get_LogoKhachHang() {
		let token = '';
		this.tokenStorage.getAccessToken().subscribe(res=>{
			token = res;
		});
		...
		// let reques3 = new Request(environment.ApiRoot + "/custemermanager/Get_LogoKhachHang", {
		// 	method: 'GET',
		// 	headers: myHeaders1
		// });
	...
	changePass(model: any): Observable<any> {
		const url = API_PRODUCTS_URL1 + `/ChangePassword`;
	...
	findYeuCauDoManhPassWord(): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		const url = API_PRODUCTS_URL2 + '/GetYeuCauDoManhPassword';
