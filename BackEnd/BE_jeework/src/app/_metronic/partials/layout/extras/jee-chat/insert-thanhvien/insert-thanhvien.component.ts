...
export class InsertThanhvienComponent implements OnInit {
    ...
    addTagName(item: any) {
        let vitri;
        const tam = Object.assign({}, item);
        this.user_tam.push(tam);
        for (let i = 0; i < this.user_tam.length; i++) {
            const index = this.itemuser.findIndex(x => x.UserName == this.user_tam[i].UserName);
            vitri = index;
        }
    ...
    selected(event: MatAutocompleteSelectedEvent): void {
        ...
        const obj = this.user_tam.find(x => x.Username == event.option.viewValue);
        if (obj) {
