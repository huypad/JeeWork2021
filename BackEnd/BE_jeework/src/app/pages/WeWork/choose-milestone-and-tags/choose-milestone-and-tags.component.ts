...
export class ChooseMilestoneAndTagComponent implements OnInit, OnChanges {
	...
	ngOnInit() {		
		...
		// if (this.item && this.showcheck) {
		// 	if (this.Loai == 'id_milestone') {
		// 		this.list.forEach(element => {
		// 			if(element.id_row==this.item.id_milestone){
		...
		// 		// this.list.forEach(element => {
		// 		// 	Tags.forEach(values => {
		// 		// 		if(values.id_row == element.id_row){
		// 		// 			element.checked=true;
		// 		// 		}
		// 		// 	});
		// 		// });
	...
	UpdateTag(_item: TagsModel) {
		...
		const _saveMessage = this.translate.instant(saveMessageTranslateParam);
		const _messageType = _item.id_row > 0 ? MessageType.Update : MessageType.Create;
		const dialogRef = this.dialog.open(TagsEditComponent, { data: { _item } });
		dialogRef.afterClosed().subscribe(res => {
			if (!res) {
				return;
			}
