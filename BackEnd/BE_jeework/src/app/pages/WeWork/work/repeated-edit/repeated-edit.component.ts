...
import { Component, OnInit, ViewChild, ChangeDetectionStrategy, ChangeDetectorRef, Inject, Input, OnChanges } from '@angular/core';
...
export class RepeatedEditComponent implements OnInit, OnChanges {
	...
	@Input() options: any = {
		showSearch: false,//hiển thị search input hoặc truyền keyword
		keyword: '',
		data: []
	};
	options_Assign: any = {
	...
	ngOnInit() {
		...
		this.id_project = this._data.id_project;
		if (this.id_project > 0 && this.id_project_team == + "") {
		...
		if (this.item.id_row > 0) {
			...
			for (var i = 0; i < this.list_weekdays.length; i++) {
				...
				this.check_weekdays.push({ Code: this.list_weekdays[i], Title: this.list_weekdays[i].Title, Checked: bool })
			}
			...
			if (this.item.Assign.id_nv) {
				this.selected_Assign.push(this.item.Assign);
				this._Assign = '@' + this.item.Assign.username;
			}
		}
		else {
			for (var i = 0; i < this.list_weekdays.length; i++) {
	...
	createForm() {
		...
		this.itemForm = this.fb.group({
			...
			id_project_team: [this.id_project_team == null ? '' : '' + this.id_project_team, Validators.required],
			frequency: [this.item.frequency == null ? '' : '' + this.item.frequency, Validators.required],
			...
			deadline: [this.item.deadline == null ? '' : this.item.deadline, Validators.required],
		});
	...
	setUpDrop_Member_Todo() {
		...
		this.filter_ctrl_todo.valueChanges
			.pipe()
			.subscribe(() => {
				this.filterMember_Todo();
			});
	...
	onSubmit(withBack: boolean = false) {
		...
		if (this.itemForm.invalid) {
			...
			this.layoutUtilsService.showActionNotification("Vui lòng nhập đầy đủ thông tin bắt buộc", MessageType.Read, 9999999999, true, true, 3000, 'top', 0);
	...
	prepare(): any {
		...
		if (!this.show_frequency)
			...
			_item.repeated_day = this.txt_repeat_day;

		var x = this.selected_Assign;
		if (this.selected_Assign.length > 0) {
			_item.assign = this.selected_Assign[0].id_nv;// chỉ lưu id
		}
		...
		if (this.selected.length > 0) {
			this.selected.map((item, index) => {
				...
				if (_true) { // Trường hợp ID_User đã tồn tại
	...
	checkShow($event, arr) {
		if ($event.target.value == "" || arr[arr.length - 1].SubTask == "")
			return;
		let item = {
			RowID: 0,
			SubTask: "",
	...
	checkShow_Todo($event, arr) {
		if ($event.target.value == "" || arr[arr.length - 1].todo == "")
			return;
		let item = {
			RowID: 0,
			todo: "",
