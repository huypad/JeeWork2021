...
import { Different_StatusesModel, MapModel } from './../../List-department/Model/List-department.model';
  ...
  IsSave=false;
	...
	InsertStatus(item) {
		this._service.InsertStatus(item).subscribe(res => {
			if (res && res.status == 1) {
				this.layoutUtilsService.showActionNotification(this.translate.instant('GeneralKey.themthanhcong'), MessageType.Read, 3000, true, false, 3000, 'top', 1);
				this.dialogRef.close(true)
			}
			else {
