...
import { WorkModel, WorkGroupModel, UpdateWorkModel } from './../../work/work.model';
...
export class WorkKanBanComponent implements OnInit {
		...
		this.ListTasks.forEach(element => {
			...
			this.listFilter.forEach(val => {
				...
				if (this.isAssignforme) {
					if (+val.id_row == +element.status && element.User.find(x=>x.id_user == this.UserID)) {
					  val.data.push(element);
					}
					else if (element.User.find(x=>x.id_user == val.id_row) && element.User.find(x=>x.id_user == this.UserID)) {
					...
					else if (element.User.find(x=>x.id_user == val.id_row) || (element.User.length == 0 && val.id_row=="") || (element.User.length > 1 && val.id_row=="0") ) {
	...
	updateDate(item) {
		...
		if (item.id_nv > 0)
			isStaff = true;
		this.UpdateByKey(item.id_row, 'deadline', moment(item.deadline).format('MM/DD/YYYY HH:mm'), isStaff)
	}
