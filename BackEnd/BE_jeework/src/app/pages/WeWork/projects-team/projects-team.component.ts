...
import { MilestoneModel, ProjectTeamDuplicateModel, ProjectTeamModel, ProjectViewsModel } from './Model/department-and-project.model';
...
export class ProjectsTeamComponent implements OnInit {
	...
	TabName: string = this.translate.instant("projects.congviecdangdanhsach") + '';
	...
	ClickShow(view_type: string) {
		...
		switch (view_type) {
			case '1': {
				this.ShowFull = false;
				this.TabName = this.translate.instant("projects.congviecdangstream") + '';
				break;
			}
			case '2': {
				this.ShowFull = false;
				this.TabName = this.translate.instant("projects.congviecdangperiod") + '';
				break;
			}
			case '3': {
				this.ShowFull = true;
				this.TabName = this.translate.instant("projects.congviecdangdanhbang") + '';
				break;
			}
			case '4': {
				this.ShowFull = false;
				this.TabName = this.translate.instant("projects.congviecdangdanhsach") + '';
				break;
			}
			case '5': {
				this.ShowFull = true;
	...
	Update_DuplicateProject(_item: ProjectTeamDuplicateModel) {
		...
		const dialogRef = this.dialog.open(DuplicateProjectComponent, { data: { _item } });
		dialogRef.afterClosed().subscribe(res => {
			if (!res) {
				this.ngOnInit();
			}
	...
	gotoPageDuplicate(item) {
		...
		const _description = this.translate.instant('notify.bancomuonchuyendentrang') + item[2];
		const _waitDesciption = this.translate.instant('notify.dangchuyentrang') + item[2];
	...
	updateStage(_item: ProjectTeamModel) {
		...
		const dialogRef = this.dialog.open(UpdateStatusProjectComponent, { data: { _item, _IsEdit: _item.IsEdit } });
		dialogRef.afterClosed().subscribe(res => {
			if (!res) {
				return;
			}
