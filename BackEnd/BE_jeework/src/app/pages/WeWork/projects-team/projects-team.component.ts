...
export class ProjectsTeamComponent implements OnInit {
  ...
  getActionActivities(value) {
    var text = "";
    if (this.language == "vi") text = value.action;
    else text = value.action_en;
    return text;
  }
  ...
  ngOnInit() {
    ...
    if (path) {
      var arr = path.split("/");
      if (arr.length > 3) this.activeLink = arr[3];
      if (
      ...
      if (res && res.status == 1) {
        ...
        this.WeWorkService.ListViewByProject(this.ID_Project).subscribe(
          (res) => {
            if (res && res.status === 1) {
              this.listDefaultView = res.data;
              var x = this.listDefaultView.find(
